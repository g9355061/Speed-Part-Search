...
- [x] **判定規則說明收合**：風險矩陣裡的兩張規則卡（庫存優先於交期、水位定義與警示）改為預設收合，標題列變成一顆「▼ 判定規則說明」按鈕。區塊 1,564px → **1,357px**。
- [x] **RSS 缺料新聞預設 5 則**：原本一次攤開 18 則。加展開鈕「▼ 顯示其餘 N 則（共 M 則）」。1,863px → **645px**。
- [x] **市場情報佐證預設 5 份**：同上做法。3,259px → **1,637px**。
- [x] **資料時間格式**：`new Date(...).toLocaleString()` 會輸出美式「8/31/2026, 12:24:24 AM」，改為 `Intl.DateTimeFormat('zh-TW', { timeZone: 'Asia/Taipei', hour12: false })` → **「2026/08/31 00:24（台北）」**。
- [x] **庫存為 0 不畫 sparkline**：那條平線佔一整欄卻零資訊，改標紅字「目前無庫存」（目前 16 列命中）。
- [x] **驗證（本機接正式站資料實測）**：
  - 桌機 11,375px → **9,297px**；手機 33,169px → **28,131px**，無水平溢位。
  - **累計自改版前：桌機 21,561 → 9,297（-57%）、手機 103,015 → 28,131（-73%）。**
  - 三個展開鈕實測可正常展開／收合（展開全部後回到 11,513px，收合鈕文字正確切換）；日期格式與「目前無庫存」以正則驗證命中。
  - `npx tsc --noEmit` ✅、`npm test` ✅、`npm run build` ✅。
- **現在最大的剩餘區塊**：缺料新聞類別矩陣 1,614px 與市場情報類別矩陣 1,354px（各 15 列的類別對照表）、料件明細 2,374px。若還要再瘦，方向是把兩張類別矩陣合併成一張（欄位本來就有重疊）。
- **修改檔案**：`src/app/demand-forecast/page.tsx`、`project_summary.md`

---

### 2026-09-05 — 缺料預測 UI 減重（第二輪）：首屏、排序分頁、區塊順序

**背景**：Danny 問頁面 UI 有無改善空間。把正式站資料複製到本機 sqlite、實際登入把頁面走過一遍後量到：桌機頁面總高 **21,561px（約 24 個螢幕）**、手機 **103,015px（約 127 個螢幕）**；料件明細一區就佔 12,456px（全頁 58%）；最該先看的「本週新變化」在 8,007px（第 9 個螢幕）；前一天才加的回測面板在 20,969px，實際上沒人看得到。Danny 同意做第一級的三項。

- [x] **Hero 瘦身**（`.forecast-hero-slim`）：移除 150 字說明段與三個統計 chips，只留標題與一句副標，261px → **159px**。
- [x] **「本週新變化 vs 長期觀察」移到首屏**：8,007px → **507px**，位置在 KPI 卡之後、週報之前。打開頁面就看得到本週要處理的 15 顆。
- [x] **料件表格風險排序 ＋ 分頁**：原本按類別編號排（C01 全在前），29 顆高風險散落在 12,456px 裡。改為依 `summary` 排序（高風險→中風險→正常→無代理商資料→尚未查詢），同風險等級內 `alertKind==='event'` 優先，再依類別與料號；每頁 25 筆並加上頁碼列（顯示「第 X–Y 筆，共 N 筆」）。篩選條件變動時自動回到第 1 頁。**12,456px → 2,374px**。
- [x] **回測面板上移**：20,969px → **3,230px**（風險矩陣之後）。
- [x] **導航列修正**：補上前一天新增面板時漏掉的「本週新變化」錨點，並依新的區塊順序重排。
- [x] **驗證（本機接正式站資料實測）**：
  - 桌機 21,561px → **11,375px（-47%）**；手機 103,015px → **33,169px（-68%）**，無水平溢位（scrollWidth === innerWidth）。
  - 分頁實測：第 1 頁 25 筆全為高風險、「顯示第 1–25 筆，共 150 筆」、1/6 頁；點「下一頁」正確跳到 26–50；點「本週新變化 15」篩選鈕後變為「顯示第 1–15 筆，共 15 筆」（頁碼有正確重設）。
  - `npx tsc --noEmit` ✅、`npm test` ✅、`npm run build` ✅。
- **尚未做（屬第二級，Danny 未指示）**：風險矩陣的規則說明（1,564px）收合、缺料新聞（1,863px）與市場情報（3,259px）改為預設顯示前 5 則 + 展開——這三塊現在合計佔改版後頁面的 60%。另有第三級細節：日期格式仍為美式（`8/31/2026, 12:24:24 AM`）、庫存為 0 的列仍畫平坦 sparkline。
- **注意**：驗證用的本機帳號（`ui-review@local.test`）與複製到 `data/users.db` 的正式站快取僅存在本機開發環境；帳號已於驗證後刪除。
- **修改檔案**：`src/app/demand-forecast/page.tsx`、`src/app/globals.css`、`project_summary.md`

---

### 2026-09-05 — 缺料預測三項改善：相對水位、狀態/事件分流、命中率回測

**背景**：Danny 要求評估缺料預測網站。對正式站資料庫做體檢（2,240 筆快照、13 週、150 顆全覆蓋）後找出四個實質問題：(a) 絕對顆數門檻（minStock 1,000／lowStock 5,000）對庫存中位數以上的料形同虛設——實測庫存分佈 P25=1,134、中位數=12,803、P90=199,494、最大 5,157 萬，跨 7 個數量級；(b) 每週 31% 亮燈，但近 6 週亮高風險的 38 顆裡有 18 顆是「6 週全紅」的結構性料（EOL/NRND/庫存長期為 0），警報疲勞；(c) 價格規則幾乎不觸發（1,984 組相鄰快照只有 11 次漲 ≥30%）；(d) 系統從未驗證過準確度。Danny 同意先做 (a)(b)(d)。

- [x] **規則合併為單一事實來源**（`src/lib/demand-forecast/risk.ts` 新）：route.ts 的 `summarizePart` 與 cache-util 的 `recalculateForecastPart` 原本各有一份規則副本且已漂移——`supplierDrop` 門檻一邊 `>=2` 一邊 `>=3`，同一顆料在 mode=full 與 mode=cached 會得到不同結論。兩者改為都呼叫 `evaluatePartRisk()`。
- [x] **(a) 相對水位門檻**：新增 `computeBaseline()` 由歷史快照算每顆料自身的庫存 P20/P40（排除 4 天內的點避免自我汙染），低於 `P20 × 0.9` 才算相對低水位（P20 定義上本來就有兩成時間會被跌破，係數 0.9 濾掉貼著門檻的抖動——實測未加係數時會出現 12,803 vs P20 12,823 這種差 20 顆就亮燈的情況）。**相對水位只參與中風險層**，高風險仍以絕對低庫存為準：未設此限時實測高風險會由 29 顆暴增到 37 顆，與「減少警報疲勞」的目標相衝突。
- [x] **(b) 狀態／事件分流**：`evaluatePartRisk` 回傳 `alertKind`（`event`＝本週趨勢觸發或風險等級較上週升高／`structural`＝持續存在的結構性狀態）與 `eventCodes`。看板新增「本週新變化 vs 長期觀察」面板與料件表格的分流篩選鈕。
- [x] **(d) 規則命中率回測**（`/api/demand-forecast/backtest` 新，12 小時快取）：用歷史快照重跑現行規則，統計每條規則亮燈後 4 週是否「回復」，並附「本週無任何訊號」對照組當基準線。刻意不用 risk_level 當結果（那本身就是規則算出來的，會循環論證），每條規則各自定義回復條件。頁面新增「規則命中率回測」面板。
- [x] **效能順帶修正**：`recalculateForecastPart` 原本每顆料各查一次 `getDemandForecastSnapshot7DaysAgo`，mode=cached 每次載入就是 150 次 DB 查詢。改為 `buildRiskContexts()` 一次查完全部歷史再分發。
- [x] **驗證（對正式站資料實跑）**：
  - 風險分佈 高29/中16/正常99/無資料6 → **高29（不變）**/中31/正常84/無資料6。新增的 15 顆全是原本不可能亮燈的大宗料相對低檔（例：GRM155R71C104KA88D 庫存 5,746,462 顆低於自身 P20 7,186,121）。
  - 分流：structural 45、event 15、正常 90。**使用者每週要看的從 60 顆降為 15 顆。**
  - 回測結果（15 週快照、150 顆、1,826 個可評估觀測點）：交期拉長 ≥8 週 **95.0%**（20 次）、庫存週減 ≥80% **73.1%**（52 次）、最低價週漲 ≥30% **72.7%**（11 次）、授權商剩 1 家 66.7%（3 次）、庫存週減 ≥50% **63.4%**（41 次）、庫存歸零 53.8%（26 次）；**對照組（本週無訊號）僅 13.8%**（1,526 次）——所有規則都明顯高於基準線，代表現行規則確實有資訊量。
  - `npx tsc --noEmit` ✅、`npm test` ✅。
- **回測帶出的下一步建議**：交期規則命中率最高（95%）卻只是「庫存低 **且** 交期長」的附屬條件，而價格規則觸發最少、命中率中等。下一版可考慮把交期升格為獨立主訊號、價格降為佐證。
- **注意**：改版首週 `event` 會偏多——`alertKind` 的「等級升高」是拿新規則的結果比對 snapshot 表裡用舊規則寫入的 risk_level。下週日 mode=full 用新規則寫入快照後即回歸正常。
- **修改檔案**：`src/lib/demand-forecast/risk.ts`（新）、`src/app/api/demand-forecast/backtest/route.ts`（新）、`src/lib/demand-forecast/cache-util.ts`、`src/app/api/demand-forecast/route.ts`、`src/app/demand-forecast/page.tsx`、`project_summary.md`
...
- [x] 正常／中風險／高風險的綠、黃、紅底色加深，三種風險標籤統一為 78 × 28 px；矩陣狀態卡同步加強色彩辨識。
- [x] 風險矩陣說明區重新整理文字層級、行距、規則編號、警示圓點與表頭副標；移除大量 emoji 與過重底線，提升掃讀性。
- [x] 料件角色改用白話名稱：「溫度計／咽喉／實戰」改為「市場指標／單一來源／實際用料」，並重寫滑鼠提示說明。
- [x] 在 150 顆料件區新增常駐「三種料件分類」說明，直接解釋市場指標、單一來源與實際用料的監測用途，手機版改為直向排列。
...
### 2026-07-24 — 風險矩陣狀態格降噪與資訊分層

- [x] 將矩陣內三欄的大型膠囊 badge 改為統一狀態卡：圖示、主狀態與判讀依據分兩層呈現。
- [x] 正常狀態改用低對比綠灰色；高／中風險才使用紅、橙強調，降低滿版「正常」標籤造成的視覺噪音。
- [x] 市場情報簡化為「未見訊號／單一來源／多源佐證」，並顯示報告佐證數；通路狀態補上已監測或需關注料件數。
- [x] 「尚未查詢」同步改為狀態卡，三個訊號欄位的寬度、對齊與互動效果一致。
- [x] 驗證：`npx tsc --noEmit` ✅、`npm test` ✅；Chrome 本機檢查 45 個矩陣狀態格完整渲染且無橫向溢位。
- **修改檔案**：`src/app/demand-forecast/page.tsx`、`src/app/globals.css`、`project_summary.md`

---

### 2026-07-24 — 缺料預測頁視覺重整

**背景**：原頁面功能完整，但首屏資訊層級接近、週報表格佔比大、說明區塊與矩陣視覺密度偏高，使用者不容易快速聚焦當週風險。

- [x] **儀表板首屏重設計**：改為深藍漸層 Hero，重新整理主標、說明、資料範圍與兩個更新動作；四個 KPI 卡片提前到週報之前，加入圖示、輔助標籤、風險色與 hover 層次。
- [x] **週報卡片化**：最新一期改為大型焦點卡，其餘歷史期數改為雙欄摘要卡，取代原本的大型三欄表格。
- [x] **矩陣與明細美化**：Panel 統一白色卡片、圓角、陰影與色彩標記；判定規則拆成兩張說明卡；矩陣與料件表加入 sticky 表頭、斑馬紋與 hover；搜尋列改為獨立工具列。
- [x] **響應式與文案同步**：新增桌面／平板／手機斷點；料件說明改為 150 顆彈性配額，不再寫「每類 10 顆」；供應商下降說明同步為 2 家降至 1 家；移除畫面上的 Markdown `**` 符號。
- [x] **驗證**：`npx tsc --noEmit` ✅、`npm test` ✅；Chrome 本機登入實測首屏、週報、風險矩陣及橫向溢位（`scrollWidth === innerWidth`）正常。
- **修改檔案**：`src/app/demand-forecast/page.tsx`、`src/app/globals.css`、`project_summary.md`
...
### 2026-07-15 — Railway 對外 IP 與華強封鎖：封鎖不會過期，重新部署才會換 IP

**背景**：修正上線後，xiaomin 在正式站實際使用時觸發了新的錯誤訊息「華強電子網暫時封鎖了本站 IP」，且**等了 2 小時仍被擋**——推翻了前一條「15–20 分鐘自動解除」的結論。

- **實測（臨時端點查 `api.ipify.org` 對外 IP，已移除）**：
  - 同一個部署內連續取樣 15 次，對外 IP **完全固定**（`152.55.176.37`）。
  - 但**每次重新部署就換一個 IP**（部署前 `162.220.232.17` → 部署後 `152.55.176.37`）。
- **修正先前的錯誤結論**：當初以為「封鎖 15–20 分鐘自動解除」，實際上是**中間部署了修正版、換到新的對外 IP** 才恢復的。**華強的封鎖不會過期**（至少 ≥2 小時，可能是永久封該 IP），所以「等一等再試」無效。
- **可用的緊急解法**：**重新部署 Railway 服務 = 拿到新的對外 IP = 立即解除封鎖**。屬於治標，新 IP 一樣會再被封。
- **對方案選擇的影響**：既然 xiaomin 正常使用就會觸發封鎖，且封鎖不會自己好，「方向 A（快取＋節流）」**不足以當解方**——只能降低頻率，擋不住該擋的。需要代理（自架或外購）讓請求從非機房 IP 出去。
- **流量實測（供代理成本估算）**：華強一頁總流量 2.97 MB／顆，其中 **JS 佔 2.3 MB**（不能擋，供應商列表靠它注入），圖片僅 507 KB，故「擋圖片省流量」只省 17%。但**被封鎖的只有 `s.hqew.com`（62 KB／顆）**，2.95 MB 的 JS/CSS/圖片都在 CDN（`res-js.hqewimg.com` 等）可直連。若只代理 `s.hqew.com`，付費流量降到 **2%**（28 顆 BOM＝1.7 MB，全代理則 84 MB）。此「選擇性代理」尚未驗證可行性（Playwright proxy 為 context 層級，需 route 攔截＋代理管道回填，可能影響反爬判斷）。
- **代理報價（2026-07 查得）**：IPRoyal 最低買 1GB／$7.35、流量不過期；DataImpulse $1/GB 但最低儲值 $50。以本專案用量，IPRoyal 1GB 約可跑 590 張 BOM（選擇性代理）。
- **尚未決定**：自架代理（用公司台灣 IP，$0，需一台常開機器）vs 外購住宅代理。待 Danny 決定後實作。
- **程式碼無改動**（僅臨時診斷端點，已移除）。

---

### 2026-07-15 — 修正 QQ 詢價「查詢第 N 筆」在正式站沒反應（靜默回空供應商）

**背景**：使用者回報正式站 `/qq-inquiry` 按「查詢第 1 筆」沒反應。畫面矛盾——綠色訊息顯示「已查詢 TXB0104YZTR；下方推薦供應商已切換為華強電子網解析結果」、進度顯示「1/28 已查」，但詢價任務清單仍是「請先查詢華強電子網」的預設列。本機同料號實測可正常抓到 3 家供應商，正式站才壞。

- **診斷過程**：因 `queriedMpns` 只在 fetch 回 200 後才寫入，可反推正式站確實回了 200 但 `suppliers: []`。本機無法登入重現、Chrome 擴充未連線，故在正式站加臨時診斷端點（`/api/hqew/diagtemp`，以 middleware 既有 `x-cron-secret` 後門放行）直接觀察 Railway 機房看到的華強頁面，測完已移除。
- **根因（兩個，皆會造成靜默空結果）**：
  1. **華強對 Railway 機房 IP 回 403 封鎖頁**——實測擷取到 `httpStatus 403`、body 僅 `您的请求过于频繁，已被网站管理员设置拦截，请联系网站客服人员进行解封！`（整份 HTML 僅 75 字元）。但 `page.goto` 對 4xx **不會拋錯**，程式從未檢查 `response.status()`，於是被當成「查詢成功但沒有供應商」照樣回 200 + 空陣列。連打 6 次即可觸發封鎖。
     - ⚠️ **本條原寫「封鎖約 15–20 分鐘自動解除」，已於 2026-07-15 稍晚證實為錯誤結論**，詳見下方「Railway 對外 IP 與華強封鎖」條目。實際上封鎖**不會過期**，當時的「恢復」是因為中間部署了新版、換到新的對外 IP。
  2. **等待條件失效**——原本 `waitForSelector('tr.ec-data, body')` 的 `body` 一定立刻命中，等於完全沒等；後面只固定 `waitForTimeout(2500)`。實測供應商列表由 JS 後續注入，**台灣快網路都要 3.2 秒**才出現，2500ms 本就不夠，Railway 更容易抓到只有骨架的 DOM（實測到 `htmlLength: 2291`、`ecData: 0` 的中間狀態）。
- [x] **修正 `src/app/api/hqew/search/route.ts`**：
  - 補檢查 `resp.status()`；403 且 body 命中 `/请求过于频繁|拦截|解封/` → 拋「華強電子網暫時封鎖了本站 IP（請求過於頻繁），請稍後再試」；其餘 4xx/5xx → 拋 `華強電子網回應 HTTP {status}`。前端既有的 `qq-inline-error` 會顯示，不再假裝成功。
  - 等待改為直接 `waitForSelector('tr.ec-data', { timeout: 20000 })`，移除無效的 `, body` 與固定 2500ms。
  - 區分**查無此料號**（頁面正常渲染、`共 0 条`、含「无结果」→ 合法空結果，不拋錯）與**被擋／改版**（列表沒出現又不是查無 → 拋錯）。
- [x] **修正前端**（`src/app/qq-inquiry/page.tsx`）：`hqewResult.suppliers` 為空時不再顯示綠色「已切換為華強解析結果」，改顯示「華強電子網查無 {料號} 的供應商（共 0 筆），請確認料號是否正確」。
- [x] **驗證**：`npx tsc --noEmit` ✅。三種情境全部實測分流正確——
  - 正常（TXB0104YZTR）：本機 `totalCount 192 / 3 家`；**正式站 Railway 實測 `supplierCount: 60`、`rowsAppeared: true`**（修正後）。
  - 查無（ZZZNOTAREALPART9988）：`totalCount 0`、不拋錯、回空結果。
  - 403 封鎖：以正式站**實際擷取的 403 原文**驗證判斷式，正確歸類為封鎖訊息（未刻意再次猛打華強以免又擋住正式站）。
- **注意**：診斷期間我的連續請求曾再次觸發華強封鎖 Railway IP，已於約 15 分鐘後自動解除、正式站現已可正常查詢。**Railway 為共用機房 IP，華強封鎖仍可能再發生**（含被同機房其他租戶連累）；此次修正是讓它**明確報錯而非靜默空白**，並未消除封鎖本身。若日後頻繁被擋，需考慮改在本機查詢（QQ 詢價的 Hammerspoon 貼上流程本就設計為本機使用）或走代理。
- **修改檔案**：`src/app/api/hqew/search/route.ts`、`src/app/qq-inquiry/page.tsx`、`project_summary.md`
- **部署**：commit `e0bb496`（修正）、`e21244e`（移除臨時診斷端點）push main，Railway 自動部署完成。
