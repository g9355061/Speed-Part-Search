...
> 最後更新：2026-07-03（QQ 詢價改回華強簽章入口流程）
...
- [x] **QQ 按鈕照第一筆安全流程**：企業企點連結仍優先使用華強解析出的簽章入口；若供應商只有個人 QQ 或未簽章入口，改為複製詢價內容後打開該料號華強頁，不再硬丟 `tencent://message` / `mqqwpa://`，避免 QQ 跳出「潛在風險」提示。
- [x] **BOM 上傳按鈕改穩定觸發**：將隱藏 input + ref click 改成原生 label 綁定 file input，避免瀏覽器環境下按鈕點擊不開檔案選擇器。
- [x] **文案同步**：QQ 操作說明改為「複製詢價內容 + 華強簽章入口 / 華強料號頁」，避免誤導使用者走 QQ 裸號直連。
