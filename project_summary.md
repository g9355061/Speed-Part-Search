...
- [x] **寬度與表格排版優化 (一頁呈現，無需右滑)**：
  ...
  - 縮減表格單元格的左右 Padding（14px -> 8px），節省了大量的橫向空間。
- [x] **新增供應商華強外部連結按鈕**：
  - 在「詢價任務清單」中的 QQ 欄位（QQ 按鈕右側）新增了一個「開啟華強頁面」的外連圖示按鈕（Icon: `external`）。
  - 點擊此按鈕會直接在新分頁中打開該供應商在華強電子網的頁面（使用 `row.jumpUrl`；若無則 fallback 到料號華強搜尋頁 `hqewSearchUrl(row.mpn)`）。
  - 將表格中第六欄（QQ 欄位）的寬度從原本優化後的 `110px` 微調為 `145px`，並縮小 `.qq-link-btn` 的最小寬度至 `94px` 配合彈性寬度，讓 QQ 按鈕與新外連圖示能完美並排顯示。
- [x] **驗證**：
