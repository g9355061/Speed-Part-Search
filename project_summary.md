...
> 最後更新：2026-06-10（週報分析與語氣升級：重構週報產生邏輯，消除程式 meta 碎碎念，分離主分析內文與原始情報片段）

---

### 2026-06-10 — 週報分析與語氣全面升級

- [x] **主客觀分離：獨立原始情報與佐證來源（Eliminated Fragmented Text Concat）**
  - 解決「一堆片段的文字湊在一起」的問題：週報故事主內文（`item.story`）不再混雜 Google 翻譯或網頁爬蟲的破碎片段。
  - 主內文重構為純淨、連貫且專業的電子供應鏈市場分析（包含市場趨勢、風險提示段落）。
  - 將原始網頁爬蟲取得的碎片原文（`item.evidence`），改以獨立區塊展示於詳情頁對應區塊下方（以灰色底框與「🔍 原始情報片段 (佐證來源)」標記），方便覆核，同時維持主內文的高級質感與流暢性。
