...
> 最後更新：2026-07-03（QQ 詢價全部統一走華強料號頁）
...
- [x] **QQ 按鈕全部照同一套流程**：不再依 `qqHref` 類型分流，也不再直接開華強簽章 QQ 或裸 QQ protocol；所有供應商 QQ 按鈕一律先複製詢價內容，再打開該料號華強頁，讓使用者從華強頁 QQ 入口進入，避免第一筆與第二筆行為不同。
- [x] **BOM 上傳按鈕改穩定觸發**：將隱藏 input + ref click 改成原生 label 綁定 file input，避免瀏覽器環境下按鈕點擊不開檔案選擇器。
- [x] **文案同步**：QQ 操作說明改為「複製詢價內容 + 華強料號頁」，避免誤導使用者以為某些供應商會直接開 QQ。
