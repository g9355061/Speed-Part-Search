...
> 最後更新：2026-07-15（修正華強查詢在正式站靜默回空供應商）
...
- **根因（兩個，皆會造成靜默空結果）**：
  1. **華強對 Railway 機房 IP 回 403 封鎖頁**——實測擷取到 `httpStatus 403`、body 僅 `您的请求过于频繁，已被网站管理员设置拦截，请联系网站客服人员进行解封！`（整份 HTML 僅 75 字元）。但 `page.goto` 對 4xx **不會拋錯**，程式從未檢查 `response.status()`，於是被當成「查詢成功但沒有供應商」照樣回 200 + 空陣列。此封鎖為**暫時性**，實測閒置約 15–20 分鐘自動解除（連打 6 次即可觸發）。
