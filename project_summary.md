...
> 最後更新：2026-06-10（週報分析與語氣升級：對接 Gemini AI 智能融合碎片資訊，消除 meta 碎碎念，主客觀分離）
...
- [x] **主客觀分離：獨立原始情報與佐證來源（Eliminated Fragmented Text Concat）**
  - 故事主內文經由 AI 融會貫通後呈現純淨專業分析。
  - 原始網頁爬蟲取得的碎片原文（`item.evidence`），改以獨立區塊展示於詳情頁對應區塊下方（以灰色底框與「🔍 原始情報片段 (佐證來源)」標記），方便覆核，同時維持主內文的高級質感與流暢性。
